# GVE DevNet Meraki Device Claim App
While it is possible to claim a device through the Meraki Dashboard, there are situations where you would like to give external people only access to a certain feature of the Meraki Dashboard. In this instance, we have a use case where implementation engineers install the Meraki devices and can claim the device to the network by entering the serial number of the device. In three steps, we can claim a device to a Meraki network:
...
![ser](IMAGES/3_claim_device.png)

4. After claiming the device, you will get a summary of the claimed device

![sum](IMAGES/4_summary.png)
...
4. Access the folder `gve_devnet_meraki_device_claim_app`

        $ cd gve_devnet_meraki_device_claim_app
